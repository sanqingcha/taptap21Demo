...
class BTS_DiscreteJump {
    ...
    NextJumpTime;
    /** 上一帧是否在空中 */
    bWasFallingLastFrame;
    /** 缓存的跳跃特效组件（复用，避免频繁创建销毁） */
    CachedJumpEffectComp = null;
    /** 活跃的定时器 ID 列表（用于清理） */
    ActiveTimers = [];
    static ProcessedPawnIDs = new Set();
    // ========== 生命周期方法 ==========
    /**
     * 服务激活时调用（节点开始执行）
     */
    ...
    PerformJump(Character, Controller, CurrentTime) {
        const CharacterMovement = Character.CharacterMovement;
        if (!CharacterMovement) {
            return;
        }
        ...
        if (this.JumpEffect) {
            ...
            if (World) {
                ...
                if (!this.CachedJumpEffectComp) {
                    ...
                    this.CachedJumpEffectComp = UE.NiagaraFunctionLibrary.SpawnSystemAttached(this.JumpEffect, MeshComp, // 附加到角色 Mesh
        ...
        if (this.bShowDebug) {
            ...
            if (World) {
                ...
                // 绘制跳跃轨迹（绿色箭头）
                UE.KismetSystemLibrary.DrawDebugArrow(World, StartLocation, EndLocation, 50.0, new UE.LinearColor(0, 1, 0, 1), 2.0, 5.0);
                // 绘制目标高度标记（黄色球体）
                UE.KismetSystemLibrary.DrawDebugSphere(World, EndLocation, 30.0, 12, new UE.LinearColor(1, 1, 0, 1), 2.0, 2.0);
            }
        }
    }
    /**
     * 落地事件处理
     */
    OnLanded(CurrentTime, BlackboardComp) {
        BlackboardComp.SetValueAsBool("bWaitingForLanding", false);
...
puerts_1.blueprint.mixin(jsCls, BTS_DiscreteJump);
//# sourceMappingURL=BTS_DiscreteJump.js.map