...
let ucls = UE.Class.Load("/Game/Game/GameMap/Blueprint/Enemy/BT_Service/BTS_FlyToTarget.BTS_FlyToTarget_C");
let jsCls = puerts_1.blueprint.tojs(ucls);
/**
 * 飞行目标移动服务（Service 版本）
 * 持续使用 SpatialPathFinding 组件计算飞行路径并移动敌人
 *
 * 【重要】这是 Service，需要在行为树中作为服务添加到节点上
 *
 * 工作流程：
 * 1. 每帧从黑板获取目标位置
...
class BTS_FlyToTarget {
    ...
    ReceiveTickAI(OwnerController, ControlledPawn, DeltaSeconds) {
        ...
        // 获取移动速度（优先从黑板读取）
        let Speed = this.DefaultMoveSpeed > 0 ? this.DefaultMoveSpeed : 600.0;
        if (this.MoveSpeedKey && this.MoveSpeedKey.SelectedKeyName) {
            const BlackboardSpeed = BlackboardComp.GetValueAsFloat(this.MoveSpeedKey.SelectedKeyName);
