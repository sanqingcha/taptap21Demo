 ...
 *
 */
class BTT_RandomMoveAroundTarget {
    // 蓝图变量
    TargetActor; // 目标Actor（黑板键）
    Distance; // 期望保持的距离
    DistanceOffset; // 距离偏移量（随机范围）
    Radius; // 选点的角度范围（度）180=半圆，360=全圆
    KeepDistanceNextLocation; // 保存计算结果的黑板键
    bShowDebug; // 是否显示调试可视化
    ReceiveExecuteAI(OwnerController, ControlledPawn) {
        ...
        const TargetActorObj = BlackboardComp.GetValueAsObject(this.TargetActor.SelectedKeyName);
        if (!TargetActorObj) {
        ...
        const ToEnemyVector = new UE.Vector(PawnLocation.X - TargetLocation.X, PawnLocation.Y - TargetLocation.Y, 0);
        // 获取这个方向的角度（度数），作为基准角度
        ...
        const RandomLocation = new UE.Vector(TargetLocation.X + OffsetX, TargetLocation.Y + OffsetY, TargetLocation.Z // 保持相同高度
        );
        // 将位置投影到NavMesh
        const ProjectedLocation = (0, puerts_1.$ref)(undefined);
        const QueryExtent = new UE.Vector(500, 500, 500);
        const bProjected = UE.NavigationSystemV1.K2_ProjectPointToNavigation(World, RandomLocation, ProjectedLocation, undefined, undefined, QueryExtent);
        let FinalLocation;
        if (bProjected && ProjectedLocation) {
            FinalLocation = (0, puerts_1.$unref)(ProjectedLocation);
        ...
        if (bShowDebug) {
            ...
            // 绘制目标周围的圆圈（红色）
            UE.KismetSystemLibrary.DrawDebugCircle(World, TargetLocation, BaseDistance, 32, new UE.LinearColor(1, 0, 0, 0.5), // 红色半透明
            DebugDuration, 2.0, new UE.Vector(0, 1, 0), new UE.Vector(1, 0, 0), false);
            // 绘制从玩家到敌人的基准方向线（白色粗线）
            const BaseDirectionEnd = new UE.Vector(TargetLocation.X + Math.cos(UE.KismetMathLibrary.DegreesToRadians(BaseAngleDeg)) * BaseDistance, TargetLocation.Y + Math.sin(UE.KismetMathLibrary.DegreesToRadians(BaseAngleDeg)) * BaseDistance, TargetLocation.Z);
            UE.KismetSystemLibrary.DrawDebugLine(World, TargetLocation, BaseDirectionEnd, new UE.LinearColor(1, 1, 1, 0.8), // 白色
            DebugDuration, 5.0);
            // 绘制允许选点的扇形边界（青色虚线）
            if (AngleRange < 360) {
                const LeftBoundaryAngle = BaseAngleDeg - HalfRange;
                const RightBoundaryAngle = BaseAngleDeg + HalfRange;
                const LeftBoundaryEnd = new UE.Vector(TargetLocation.X + Math.cos(UE.KismetMathLibrary.DegreesToRadians(LeftBoundaryAngle)) * BaseDistance, TargetLocation.Y + Math.sin(UE.KismetMathLibrary.DegreesToRadians(LeftBoundaryAngle)) * BaseDistance, TargetLocation.Z);
                const RightBoundaryEnd = new UE.Vector(TargetLocation.X + Math.cos(UE.KismetMathLibrary.DegreesToRadians(RightBoundaryAngle)) * BaseDistance, TargetLocation.Y + Math.sin(UE.KismetMathLibrary.DegreesToRadians(RightBoundaryAngle)) * BaseDistance, TargetLocation.Z);
                UE.KismetSystemLibrary.DrawDebugLine(World, TargetLocation, LeftBoundaryEnd, new UE.LinearColor(0, 1, 1, 0.6), // 青色
                DebugDuration, 2.0);
                UE.KismetSystemLibrary.DrawDebugLine(World, TargetLocation, RightBoundaryEnd, new UE.LinearColor(0, 1, 1, 0.6), // 青色
                DebugDuration, 2.0);
            }
            // 绘制从目标到计算位置的线（黄色）
            UE.KismetSystemLibrary.DrawDebugLine(World, TargetLocation, FinalLocation, new UE.LinearColor(1, 1, 0, 1), // 黄色
            DebugDuration, DebugThickness);
            // 绘制目标位置点（绿色球体）
            UE.KismetSystemLibrary.DrawDebugSphere(World, FinalLocation, 50.0, 12, new UE.LinearColor(0, 1, 0, 1), // 绿色
            DebugDuration, DebugThickness);
            // 绘制从AI到目标位置的线（蓝色半透明）
            UE.KismetSystemLibrary.DrawDebugLine(World, PawnLocation, FinalLocation, new UE.LinearColor(0, 0, 1, 0.5), // 蓝色半透明
            DebugDuration, 1.0);
        }
        this.FinishExecute(true);
    }
}
puerts_1.blueprint.mixin(jsCls, BTT_RandomMoveAroundTarget);
//# sourceMappingURL=BTT_RandomMoveAroundTarget.js.map