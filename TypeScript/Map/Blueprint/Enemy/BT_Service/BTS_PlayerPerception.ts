...
let jsCls = blueprint.tojs<typeof UE.Game.Game.GameMap.Blueprint.Enemy.BT_Service.BTS_PlayerPerception.BTS_PlayerPerception_C>(ucls);
interface BTS_PlayerPerception extends UE.Game.Game.GameMap.Blueprint.Enemy.BT_Service.BTS_PlayerPerception.BTS_PlayerPerception_C { }
 ...
 */
class BTS_PlayerPerception implements BTS_PlayerPerception {
    DetectionRadius: UE.BlackboardKeySelector;// 检测半径
    SightAngle: number;// 检测角度
    EyeHeight: number// 眼睛高度
    bShowDebug: boolean;
    ...
    ReceiveTickAI(OwnerController: $Nullable<UE.AIController>, ControlledPawn: $Nullable<UE.Pawn>, DeltaSeconds: number): void {
        ...
        if (bShowDebug) {
            ...
            if (SightAngle < 360.0) {
                ...
                const LeftRotation = UE.KismetMathLibrary.ComposeRotators(
                    AIRotation,
                    new UE.Rotator(0, LeftBoundaryAngle, 0)
                );
                const LeftDirection = UE.KismetMathLibrary.GetForwardVector(LeftRotation);
                const LeftEnd = AIPawnLocation.op_Addition(
                    UE.KismetMathLibrary.Multiply_VectorFloat(LeftDirection, DetectionRange)
                );

                // 右边界
                const RightRotation = UE.KismetMathLibrary.ComposeRotators(
                    AIRotation,
                    new UE.Rotator(0, RightBoundaryAngle, 0)
                );
                const RightDirection = UE.KismetMathLibrary.GetForwardVector(RightRotation);
                const RightEnd = AIPawnLocation.op_Addition(
                    UE.KismetMathLibrary.Multiply_VectorFloat(RightDirection, DetectionRange)
                );
                ...
                const CenterEnd = AIPawnLocation.op_Addition(
                    UE.KismetMathLibrary.Multiply_VectorFloat(AIForwardVector2, DetectionRange)
                );
                UE.KismetSystemLibrary.DrawDebugLine(
                    World,
                    AIPawnLocation,
                    CenterEnd,
                    new UE.LinearColor(1, 0, 0, 1), // 红色
                    DebugDrawDuration,
                    LineThickness
                );
                ...
                for (let i = 0; i < ArcSegments; i++) {
                    ...
                    const CurrentRotation = UE.KismetMathLibrary.ComposeRotators(
                        AIRotation,
                        new UE.Rotator(0, CurrentAngle, 0)
                    );
                    const CurrentDirection = UE.KismetMathLibrary.GetForwardVector(CurrentRotation);
                    const CurrentPoint = AIPawnLocation.op_Addition(
                        UE.KismetMathLibrary.Multiply_VectorFloat(CurrentDirection, DetectionRange)
                    );

                    const NextRotation = UE.KismetMathLibrary.ComposeRotators(
                        AIRotation,
                        new UE.Rotator(0, NextAngle, 0)
                    );
                    const NextDirection = UE.KismetMathLibrary.GetForwardVector(NextRotation);
                    const NextPoint = AIPawnLocation.op_Addition(
                        UE.KismetMathLibrary.Multiply_VectorFloat(NextDirection, DetectionRange)
                    );
                ...
                UE.KismetSystemLibrary.DrawDebugCircle(
                    World,
                    CircleCenter,
                    CircleRadius * 0.9, // 稍小一点的内圈
                    32,
                    new UE.LinearColor(1, 0, 0, 0.3), // 红色半透明
                    DebugDrawDuration,
                    3.0,
                    new UE.Vector(0, 1, 0),
                    new UE.Vector(1, 0, 0),
        ...
        const OverlappedActors = $ref(UE.NewArray(UE.Actor));
        const ActorsToIgnore = UE.NewArray(UE.Actor);
        ActorsToIgnore.Add(ControlledPawn); // 忽略敌人自己

        const bOverlapFound = UE.KismetSystemLibrary.SphereOverlapActors(
            World,
            AIPawnLocation,
            DetectionRange,
            UE.NewArray(UE.EObjectTypeQuery), // 空数组，检测所有类型
        ...
        if (bOverlapFound && OverlappedActors) {
            const Actors = $unref(OverlappedActors);
        ...
        const HitResult = $ref<UE.HitResult>(undefined);
     ...
     * 保存最后已知位置，然后清除目标
     */
    private HandleLostPlayer(BlackboardComp: UE.BlackboardComponent): void {
        // 获取当前的TargetActor
        ...
        if (CurrentTarget) {
            ...
            if (!ExistingLocation || (ExistingLocation.X === 0 && ExistingLocation.Y === 0 && ExistingLocation.Z === 0)) {
                const TargetActor = CurrentTarget as UE.Actor;
                if (TargetActor) {
...
blueprint.mixin(jsCls, BTS_PlayerPerception);
