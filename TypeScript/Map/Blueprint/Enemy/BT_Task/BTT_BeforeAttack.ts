...
let jsCls = blueprint.tojs<typeof UE.Game.Game.GameMap.Blueprint.Enemy.BT_Task.BTT_BeforeAttack.BTT_BeforeAttack_C>(ucls);
interface BTT_BeforeAttack extends UE.Game.Game.GameMap.Blueprint.Enemy.BT_Task.BTT_BeforeAttack.BTT_BeforeAttack_C { }

class BTT_BeforeAttack implements BTT_BeforeAttack {
    // 私有字段：保存定时器和初始值
    private ActiveTimers: number[] = [];
    private InitialEmissionValue: number = 1.0;
    private DynamicMaterial: UE.MaterialInstanceDynamic | null = null;
    ...
    private StartEmissionFade(Material: UE.MaterialInstanceDynamic, InitialValue: number): void {
    ...
    private ResetEmissionAndFinish(Material: UE.MaterialInstanceDynamic, InitialValue: number): void {
        // 重置材质参数
...
blueprint.mixin(jsCls, BTT_BeforeAttack);
