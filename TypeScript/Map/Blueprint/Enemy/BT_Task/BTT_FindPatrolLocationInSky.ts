...
let jsCls = blueprint.tojs<typeof UE.Game.Game.GameMap.Blueprint.Enemy.BT_Task.BTT_FindPatrolLocationInSky.BTT_FindPatrolLocationInSky_C>(ucls);
interface BTT_FindPatrolLocationInSky extends UE.Game.Game.GameMap.Blueprint.Enemy.BT_Task.BTT_FindPatrolLocationInSky.BTT_FindPatrolLocationInSky_C { }

class BTT_FindPatrolLocationInSky implements BTT_FindPatrolLocationInSky {
    /** 水平搜索半径 */
    SearchRadius: number;
    ...
    ReceiveExecuteAI(OwnerController: $Nullable<UE.AIController>, ControlledPawn: $Nullable<UE.Pawn>): void {
        ...
        for (let i = 0; i < MaxAttempts; i++) {
            // 生成候选位置
            const CandidateLocation = this.GenerateRandomLocation(
                CurrentLocation,
                BaseHeight,
                SearchRadius,
                MinHeightOffset,
    ...
    private IsLocationValid(
        ...
        const OutActors = $ref(UE.NewArray(UE.Actor));

        const bOverlap = UE.KismetSystemLibrary.SphereOverlapActors(
            World,
            Location,
            CheckRadius,
            ObjectTypes,
            UE.Actor.StaticClass(),  // 检测所有 Actor
            ActorsToIgnore,
            OutActors
        );
    ...
    private DrawDebugInfo(
        World: UE.World,
        CurrentLocation: UE.Vector,
        PatrolLocation: UE.Vector,
        SearchRadius: number,
        BaseHeight: number,
        MinHeightOffset: number,
        MaxHeightOffset: number
    ...
    private VectorToString(Vec: UE.Vector): string {
        return `(X=${Vec.X.toFixed(0)}, Y=${Vec.Y.toFixed(0)}, Z=${Vec.Z.toFixed(0)})`;
    }
}

blueprint.mixin(jsCls, BTT_FindPatrolLocationInSky);